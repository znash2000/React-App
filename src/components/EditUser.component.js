...
const EditUser = (props) => {
  ...
  const handleSubmit = (userObject) => {
    axios.put("http://localhost:3500/users/update-user/" + props.match.params.id, userObject)
      .then((res) => {
        if (res.status === 200) {
  ...
  useEffect(() => {
    axios.get("http://localhost:3500/users/update-user/" + props.match.params.id)
      .then((res) => {
        const { name, username, email, password, role, status } = res.data;
        setName('');
            setUsername('');
            setEmail('');
            setRole('');
        ...
        <div className="register">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Form.Group>
              ...
              <Form.Control type="checkbox" checked={this.state.status} />
            </Form.Group>
